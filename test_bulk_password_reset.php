...
// Test bulk password reset functionality
$baseUrl = 'http://127.0.0.1:8000';

echo "Testing Bulk Password Reset Functionality\n";
echo "========================================\n\n";
...
// Step 4: Check existing users to reset passwords for
echo "4. Checking existing users for password reset...\n";

// Create a simple PHP script to get existing users
$getUsersScript = '<?php
...
echo "Available users for password reset:\n";
$userIds = [];
...
file_put_contents('get_users_for_reset.php', $getUsersScript);

// Execute the user check
$getUsersOutput = shell_exec('php get_users_for_reset.php 2>&1');
echo $getUsersOutput;

// Extract user IDs from output
preg_match('/User IDs: (.+)/', $getUsersOutput, $matches);
$userIds = isset($matches[1]) ? trim($matches[1]) : '';

if (empty($userIds)) {
    echo "⚠️ No users found for password reset testing\n";
    echo "Creating test users first...\n";
...
$testUsers = [
    ["name" => "Test User 1", "email" => "testuser1@test.com", "role" => "teacher"],
    ["name" => "Test User 2", "email" => "testuser2@test.com", "role" => "teacher"],
];
...
';
    ...
    file_put_contents('create_test_users.php', $createUsersScript);
    $createUsersOutput = shell_exec('php create_test_users.php 2>&1');
...
// Step 5: Test bulk password reset
echo "5. Testing bulk password reset...\n";
...
if ($resetHttpCode === 200) {
    ...
    if ($jsonResponse !== null) {
        ...
        if (isset($jsonResponse['reset_count'])) {
            echo "   Passwords Reset: " . $jsonResponse['reset_count'] . " users\n";
        }
...
if (file_exists('get_users_for_reset.php')) {
    unlink('get_users_for_reset.php');
}
if (file_exists('create_test_users.php')) {
    unlink('create_test_users.php');
}
