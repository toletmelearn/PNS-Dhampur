...
class UpdateStudentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'reason_for_update' => 'nullable|string|max:500',
            'updated_by_notes' => 'nullable|string|max:1000',
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'roll_number.unique' => 'This roll number is already assigned to another student in the selected class and section.',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $studentId = $this->route('student') ?? $this->route('id');
            if ($studentId && $this->has('status')) {
                $currentStudent = \App\Models\Student::find($studentId);
                if ($currentStudent) {
                    $this->validateStatusTransition($validator, $currentStudent->status, $this->status);
                }
            }
        });
    }
    ...
    {
        $allowedTransitions = [
            'active' => ['inactive', 'transferred', 'graduated', 'suspended'],
            'inactive' => ['active', 'transferred', 'graduated'],
        ...
        $requiresReason = ['suspended', 'transferred', 'graduated', 'inactive'];
        if (in_array($newStatus, $requiresReason) && $currentStatus !== $newStatus && empty($this->reason_for_update)) {
            $validator->errors()->add('reason_for_update', "Reason is required when changing status to '{$newStatus}'.");
        }
    }
}